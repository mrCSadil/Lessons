...
class Archer(Hero):
    ...
    def __init__(self, name, health, arrows=30):
        super().__init__(name, health)
        self.arrows = arrows
    ...
    def shoot(self):
        if self.arrows >= 1:
            self.arrows -= 1
            return f"{self.name} you shoot \n you have {self.arrows}"
        else:
            return f"you have no arrows"

    def action(self):
        base_action = super().action()
        special_result = self.shoot()
        return f"{base_action} {special_result}"

archer = Archer("Adil", 100)
